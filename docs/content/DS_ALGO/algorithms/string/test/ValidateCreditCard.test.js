...
describe('Validate credit card number', () => {
    ...
    it('should throw an error on credit card with invalid length', () => {
        ...
        ccWithInvalidLength.forEach((invalidCC) =>
            expect(() => validateCreditCard(invalidCC)).toThrow(`${invalidCC} is an invalid credit card number because ` + 'of its length.')
        );
    });
    it('should throw an error on credit card with invalid start substring', () => {
        ...
        ccWithInvalidStartSubstring.forEach((invalidCC) =>
            expect(() => validateCreditCard(invalidCC)).toThrow(`${invalidCC} is an invalid credit card number because ` + 'of its first two digits.')
        );
    });
    it('should throw an error on credit card with luhn check fail', () => {
        ...
        invalidCCs.forEach((invalidCC) =>
            expect(() => validateCreditCard(invalidCC)).toThrow(`${invalidCC} is an invalid credit card number because ` + 'it fails the Luhn check.')
        );
    });
});
