...
export default class RadixSort extends Sort {
  ...
  placeElementsInNumberBuckets(array, index) {
    // See below. These are used to determine which digit to use for bucket
    // allocation
    ...
    array.forEach((element) => {
      ...
      if (element < divided) {
        ...
        /**
         * Say we have element of 1,052 and are currently on index 1 (starting
         * from 0). This means we want to use '5' as the bucket. `modded` would
         * be 10 ** (1 + 1), which is 100. So we take 1,052 % 100 (52) and
         * divide it by 10 (5.2) and floor it (5).
         */
  ...
  getCharCodeOfElementAtIndex(element, index, numPasses) {
    ...
    /**
     * If each character has been organized, use first character to determine
     * bucket, otherwise iterate backwards through element
     */
  ...
  /**
   * Number of passes is determined by the length of the longest element in the
  ...
  createBuckets(numBuckets) {
     ...
     * an array prevents each bucket from containing a reference to the same
     * array
     */
