...
console.log(
    '[inject-netlify-identity-widget.js] injecting netlify-cms scripts');
...
function injectHtmlBeforeHtml(html, beforeHtml, injectHtml) {
  ...
  if (index === -1) {
    throw new Error(`[inject-netlify-identity-widget.js] ${
        beforeHtml} was not found in ${indexHTMLFilePath}`);
  }
...
indexHTMLData =
    injectHtmlBeforeHtml(indexHTMLData, '</body>',
