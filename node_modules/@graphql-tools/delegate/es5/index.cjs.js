...
function cloneSubschemaConfig(subschemaConfig) {
    var newSubschemaConfig = tslib.__assign(tslib.__assign({}, subschemaConfig), { transforms: subschemaConfig.transforms != null ? tslib.__spreadArray([], tslib.__read(subschemaConfig.transforms)) : undefined });
    if (newSubschemaConfig.merge != null) {
        ...
        Object.keys(newSubschemaConfig.merge).forEach(function (typeName) {
            var mergedTypeConfig = (newSubschemaConfig.merge[typeName] = tslib.__assign({}, subschemaConfig.merge[typeName]));
            ...
            if (mergedTypeConfig.fields != null) {
                var fields_1 = (mergedTypeConfig.fields = tslib.__assign({}, mergedTypeConfig.fields));
...
function mergeFields(mergedTypeInfo, typeName, object, fieldNodes, sourceSubschemaOrSourceSubschemas, targetSubschemas, context, info) {
    ...
    delegationMap.forEach(function (selectionSet, s) {
        ...
        if (isPromise(maybePromise)) {
            ...
            maybePromise = maybePromise.then(undefined, function (error) { return error; });
        }
...
function expandAbstractTypes(targetSchema, possibleTypesMap, reversePossibleTypesMap, interfaceExtensionsMap, document) {
    ...
    var newDocument = tslib.__assign(tslib.__assign({}, document), { definitions: tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(operations)), tslib.__read(newFragments)) });
...
function filterToSchema(targetSchema, document, variables) {
    ...
    return {
        document: {
            kind: graphql.Kind.DOCUMENT,
            definitions: tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(newOperations)), tslib.__read(newFragments)),
        },
        variables: newVariables,
    };
}
...
function checkResultAndHandleErrors(result, context, info, responseKey, subschema, returnType, skipTypeMerging, onLocatedError) {
    ...
    if (returnType === void 0) { returnType = info.returnType; }
    var _a = mergeDataAndErrors(result.data == null ? undefined : result.data[responseKey], result.errors == null ? [] : result.errors, info ? graphql.responsePathAsArray(info.path) : undefined, onLocatedError), data = _a.data, unpathedErrors = _a.unpathedErrors;
...
function mergeDataAndErrors(data, errors, path, onLocatedError, index) {
    ...
    if (data == null) {
        ...
        var newError = graphql.locatedError(new AggregateError(errors), undefined, path);
...
function addVariablesToRootField(targetSchema, originalRequest, args) {
    ...
    return {
        document: tslib.__assign(tslib.__assign({}, document), { definitions: tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(newOperations)), tslib.__read(fragments)) }),
