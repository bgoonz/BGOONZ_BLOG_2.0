...
function performIndexScan(context) {
  ...
  for (const typeName of context.nodeTypeNames) {
    ...
    if (canUseIndexForSorting(indexMetadata, sortFields)) {
      const nodes = filterNodes(context, indexMetadata); // Interleave nodes of different types (not expensive for already sorted chunks)
...
function filterNodes(context, indexMetadata) {
  ...
  return completeFiltering(context, nodes, usedQueries);
}
/**
