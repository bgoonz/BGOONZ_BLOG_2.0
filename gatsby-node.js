...
async function runIndexQueries(
        ...
        for (const [i, {
            ...
            for (const [id, existingObj] of Object.entries(indexedObjects)) {
                if (queryResultsMap.hasOwnProperty(id)) {
                    ...
                    if (matchFields.some(field => !deepEqual(existingObj[field], newObj[field], {
                            strict: true
                        }))) {
    ...
    if (objectsToIndex.length) {
        ...
        const chunkJobs = chunks.map(async function(chunked) {
            const {
                taskID
            } = await indexToUse.addObjects(chunked);
    ...
    const {
        settings = mainSettings, forwardToReplicas
    } = queries[0] || {};
