...
const getStats = (publicDir) => {
    if (cachedStats) {
        ...
        cachedStats = JSON.parse(fs.readFileSync(nodePath.join(publicDir, `webpack.stats.json`), `utf-8`));
...
export default async function staticPage(pagePath, isClientOnlyPage, publicDir, error, callback) {
    let bodyHtml = ``;
    ...
    const generateBodyHTML = async () => {
        ...
        let scriptsAndStyles = flatten(
            [`commons`].map((chunkKey) => {
                ...
                namedChunkGroups[chunkKey].assets.forEach((asset) => chunks.push({ rel: `preload`, name: asset.name }));
        ...
        styles
            ...
            .forEach((style) => {
                headComponents.unshift(
                    <link data-identity={`gatsby-dev-css`} key={style.name} rel="stylesheet" type="text/css" href={`${__PATH_PREFIX__}/${style.name}`} />
                );
            });
        ...
        const bodyComponent = apiRunner(`wrapRootElement`, { element: routerElement, pathname: pagePath }, routerElement, ({ result }) => {
    ...
    const htmlElement = React.createElement(Html, {
        ...
        postBodyComponents: postBodyComponents.concat(
            [
                !BROWSER_ESM_ONLY && <script key={`polyfill`} src="/polyfill.js" noModule={true} />,
