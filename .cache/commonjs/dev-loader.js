...
function mergePageEntry(cachedPage, newPageData) {
    return {
        ...cachedPage,
        payload: { ...cachedPage.payload, json: newPageData.result, page: { ...cachedPage.payload.page, staticQueryResults: newPageData.staticQueryResults } }
    };
}
...
class DevLoader extends _loader.BaseLoader {
    ...
    handlePageQueryResultHotUpdate(msg) {
        ...
        const cachedPageData =
            (_this$pageDataDb$get = this.pageDataDb.get(pageDataDbCacheKey)) === null || _this$pageDataDb$get === void 0
                ? void 0
    ...
    handleStalePageDataMessage(msg) {
        msg.payload.stalePageDataPaths.forEach((dirtyQueryId) => {
            ...
            if (cachedPage) {
                // if we have page data in cache, mark it as stale
                this.pageDb.set(normalizedId, { ...cachedPage, payload: { ...cachedPage.payload, stale: true } });
            }
        });
    }
}
