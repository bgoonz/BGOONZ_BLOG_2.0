...
apiRunnerAsync(`onClientEntry`).then(() => {
    ...
    fetch(`/___services`)
        ...
        .then((services) => {
            if (services.developstatusserver) {
                ...
                const parentSocket = io(`${window.location.protocol}//${window.location.hostname}:${services.developstatusserver.port}`);
    ...
    const renderer = apiRunner(`replaceHydrateFunction`, undefined, defaultRenderer)[0];
        ...
        function onHydrated() {
            ...
            if (process.env.GATSBY_QUERY_ON_DEMAND_LOADING_INDICATOR && process.env.GATSBY_QUERY_ON_DEMAND_LOADING_INDICATOR === `true`) {
                ...
                if (renderer === ReactDOM.hydrateRoot) {
                    ReactDOM.createRoot(indicatorMountElement).render(<LoadingIndicatorEventHandler />);
                } else {
