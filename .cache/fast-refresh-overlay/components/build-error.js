...
const BuildError = ({ error, open, dismiss }) => {
    ...
    const lineNumberFiltered = detailedError.filter((d) => d.content !== ` ` && d.content.match(lineNumberRegex))[0]?.content;
    ...
    const header = (
        <>
            ...
            <button onClick={() => open(file, lineNumber)} data-gatsby-overlay="header__open-in-editor">
                Open in editor
            </button>
        </>
    );
