...
const RuntimeError = ({ error, open, dismiss }) => {
    ...
    const { moduleId, lineNumber, columnNumber, functionName } = getCodeFrameInformation(stacktrace);
    ...
    const header = (
        <>
            ...
            <button onClick={() => open(moduleId, res.sourcePosition.line)} data-gatsby-overlay="header__open-in-editor">
                Open in editor
            </button>
        </>
    );
