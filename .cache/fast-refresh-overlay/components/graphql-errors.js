        ...
        <AccordionItem open={open} title={title}>
            <div data-gatsby-overlay="body__graphql-error-message">
                <div data-gatsby-overlay="codeframe__top">
                    ...
                    <button data-gatsby-overlay="body__open-in-editor" onClick={() => openInEditor(filePath, lineNumber)}>
                        Open in Editor
                    </button>
                </div>
                {filePath && (
        ...
        <Overlay>
            <Header data-gatsby-error-type="graphql-error">
                <div data-gatsby-overlay="header__cause-file">
                    <h1 id="gatsby-overlay-labelledby">{hasMultipleErrors ? `${errors.length} Unhandled GraphQL Errors` : `Unhandled GraphQL Error`}</h1>
                </div>
            ...
            <Body>
                <p data-gatsby-overlay="body__describedby" id="gatsby-overlay-describedby">
                    ...
                    {hasMultipleErrors ? `errors` : `error`} found in your files. See the list below to fix {hasMultipleErrors ? `them` : `it`}:
                </p>
                <Accordion>
                    {deduplicatedErrors.map((error, index) => (
                        <WrappedAccordionItem open={index === 0} error={error} key={`${error.sourceMessage}-${index}`} />
                    ))}
                </Accordion>
            </Body>
        </Overlay>
