    ...
    return (
        <AccordionItem open={open} title={<Title />}>
            ...
            <div data-gatsby-overlay="codeframe__top">
                ...
                <button data-gatsby-overlay="body__open-in-editor" onClick={() => openInEditor(filePath, line)}>
                    Open in Editor
                </button>
            </div>
        ...
        <Overlay>
            <Header data-gatsby-error-type="runtime-error">
                <div data-gatsby-overlay="header__cause-file">
                    <h1 id="gatsby-overlay-labelledby">{hasMultipleErrors ? `${errors.length} Unhandled Runtime Errors` : `Unhandled Runtime Error`}</h1>
                </div>
            ...
            <Body>
                ...
                <Accordion>
                    {deduplicatedErrors.map((error, index) => (
                        <WrappedAccordionItem open={index === 0} key={`${error.message}-${index}`} error={error} />
                    ))}
                </Accordion>
            </Body>
        </Overlay>
