...
function findFileNode({
    ...
    while (fileNode && fileNode.internal.type !== `File` && fileNode.parent) {
        ...
        if (!fileNode) {
            break;
        } else(_.includes(ids, fileNode.id)) {
...
exports.createPages = ({ graphql, getNode, actions, getNodesByType }) => {
  ...
  // underlying node.
  return graphql(` >>>
        >>> > 0951 c0be15022ef1b1179781caa3e833be1768d0 {
            allMarkdownRemark {
                edges {
                    node {
                        id
        ...
        `);
            ...
            const component = path.resolve(`. / src / templates / $ {
            template
        }.js `);
            if (!fs.existsSync(component)) {
                console.error(`
        Error: component "src/templates/${template}.js"
        missing
        for $ {
            url
        }
