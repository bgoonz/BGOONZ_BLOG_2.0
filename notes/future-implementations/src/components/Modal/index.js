    ...
    if (open)
        return (
            <ModalBehind open={open} onClick={() => setModal()}>
                <ModalDiv onClick={(event) => event.stopPropagation()} {...{ className, fullscreen, ref }}>
                    <Close onClick={() => setModal()} />
                    <FullscreenToggle onClick={() => setFullscreen(!fullscreen)} {...{ fullscreen }} />
                    {showArrows && (
                        <>
        ...
        if (typeof document !== `undefined`) document.body.style.removeProperty(`overflow-y`);
