...
Promise.resolve()
    .then(async () => {
        ...
        for (let postpath of blogPosts) {
            const [year, month, day, title] = postpath.replace('src/pages/blog/', '').split('/');
        ...
        const sorted = routes.sort((post1, post2) => (parseISO(post1.date) > parseISO(post2.date) ? -1 : 1));
        const blogManifest = {
            routes: sorted
        };
        ...
        await fs.writeFile(path.resolve('./src/blogIndex.json'), JSON.stringify(blogManifest, null, 2));
        await fs.writeFile(path.resolve('./src/blogIndexRecent.json'), JSON.stringify(blogRecentSidebar, null, 2));
    })
    .catch(console.error);
