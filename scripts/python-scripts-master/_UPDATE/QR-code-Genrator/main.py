...
class Qr_Genrator:
    def __init__(self, root):
        ...
        lbl_emp_code = Label(
            emp_Frame, text="  Employee ID", font=("time new roman", 15), bg="white"
        ).place(x=20, y=60)
        lbl_emp_name = Label(
            emp_Frame, text="  Name", font=("time new roman", 15), bg="white"
        ).place(x=20, y=100)
        lbl_emp_dept = Label(
            emp_Frame, text="  Department", font=("time new roman", 15), bg="white"
        ).place(x=20, y=140)
        lbl_emp_designation = Label(
            emp_Frame, text="  Designation", font=("time new roman", 15), bg="white"
        ).place(x=20, y=180)
        ...
        btn_clear = Button(
            ...
            bg="#2196f3",
            fg="white",
        ).place(x=290, y=250, width=120, height="30")
    ...
    def genrate(self):
        if (
            ...
            qr_code.save(
                "./QR-code-Genrator/employee_qr/emp_"
                + str(self.var_emp_code.get() + ".png")
            )
            # qr code img update
            self.im = ImageTk.PhotoImage(
                file="../QR-code-Genrator/employee_qr/emp_"
                + str(self.var_emp_code.get() + ".png")
            )
