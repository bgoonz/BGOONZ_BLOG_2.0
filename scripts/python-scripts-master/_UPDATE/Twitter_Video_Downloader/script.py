...
def Download_vid():
    ...
    with subprocess.Popen(
        "youtube-dl {} --no-cache-dir".format(url),
...
Download_button = Button(
    main, text="Download", font=("Calibri", 9), command=Download_vid
)
Download_button.place(x=250, y=80, width=100)

Download_Window = Text(
    main, font=("Calibri", 9), bg="black", fg="white", bd=1, relief=SUNKEN, wrap=WORD
)
Download_Window.insert(
    END,
    "Welcome to Twitter Video Downloader, Provide a Twitter video link in the above box and click download to start the process. :D",
)
