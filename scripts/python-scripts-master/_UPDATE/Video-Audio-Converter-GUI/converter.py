...
def save_audio_file():
    ...
    audio_filepath.set(
        filedialog.asksaveasfilename(
            ...
            title="Select your audio file directory",
            filetypes=[("MP3 File", "*.mp3"), ("Wave File", "*.wav")],
        )
    )
    try:
...
Button(UI_frame, text="Browse", command=get_video_file, bg="grey").grid(
    row=1, column=2, padx=5, pady=5
)
Button(UI_frame, text="Convert", command=run_program, bg="green").grid(
