...
labelweight = Label(
    f1a, text="Select Weight in Kilograms", font=("arial", 20, "bold"), bd=20
).grid(row=0, column=0)
Bodyweight = Scale(
    f1a, variable=var1, from_=1, to=500, length=880, tickinterval=30, orient=HORIZONTAL
)
Bodyweight.grid(row=1, column=0)

labelheight = Label(
    f1b, text="Enter Height in Meters Square", font=("arial", 20, "bold"), bd=20
).grid(row=0, column=0)
textheight = Entry(
    f1b,
    textvariable=var2,
...
labelBMITable = Label(f2, font=("arial", 20, "bold"), text="BMI Table").grid(
    row=0, column=0
)
