...
class SlideShare:
    ...
    def get_slides(self, download_url=None, filename=None):
        if download_url:
            ...
            i_dir = self.download_images(
                input('SlideShare full URL (including "http://"): ')
            )
        if filename:
    ...
    def download_images(page_url):
        ...
        )  # Parse out the slide images
        image_dir = (
            soup.title.string.strip(" \t\r\n").lower().replace(" ", "-")
        )  # Get name of the slide deck
        try:
            mkdir(image_dir)  # Create the folder for our images
        except FileExistsError:
            print(
                "The directory '%s' already exists. Assuming PDF rebuild, continuing with existing contents...\n"
