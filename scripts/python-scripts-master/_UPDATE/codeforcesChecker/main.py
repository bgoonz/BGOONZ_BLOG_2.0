...
def getUrlforContest():  # function to get URL for contests
    print(
        "Enter the URL of the page(Example:https://codeforces.com/contest/1328):  ",
        end="",
    )
    url = input()
    if len(url) < 30:
...
def getUrlforSingleProblem():  # function to get URL for a  problem from problemset
    print(
        "Enter the URL of the page(Example:https://codeforces.com/problemset/problem/1333/E):  ",
        end="",
    )
    url = input()
    if len(url) < 43:
...
if choice == 1:  # for complete contest
    ...
    elements = browser.find_elements_by_xpath(
        final
    )  # get all required elements by xpath
    # print(elements)
    length = len(elements)
    length = length // 2
    ...
    os.makedirs(
        os.path.join(os.getcwd(), contestFolder)
    )  # make directory for the contest
    ...
    for i in range(0, len(elements), 2):
        ...
        os.makedirs(
            os.path.join(currentDirectory, toadd)
        )  # make directory for individual problems
        ...
        shutil.copy(
            os.path.join(intialDirectory, "template.cpp"),
            os.path.join(os.getcwd(), "soln.cpp"),
        )
        shutil.copy(
            os.path.join(intialDirectory, "checker.py"),
