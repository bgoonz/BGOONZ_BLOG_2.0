...
for article in articles:
    article_data = article.find("a", class_="crayons-story__hidden-navigation-link")[
        "href"
    ]
    ...
    title_string = (
        (title_content.text.strip()).encode("latin-1", "replace").decode("latin-1")
    )
    author_string = (
        ("By - {}".format(author_name.text.strip()))
    ...
    for p_tag in p_tags:
        article_part = (
            (p_tag.text.strip()).encode("latin-1", "replace").decode("latin-1")
        )
