...
def sql_insert(con, entities):
    cursorObj = con.cursor()
    cursorObj.execute(
        "INSERT INTO users(name, profile_link, date_range, repo, repo_lang, repo_link) VALUES(?, ?, ?, ?, ?, ?)",
        entities,
    )
    con.commit()
...
def sql_fetch(con):
    ...
    try:
        cursorObj.execute(
            "SELECT DISTINCT * FROM users ORDER BY rowid DESC"
        )  # SQL search query
    except Error:
    ...
    first_row = (
        "{:^30}".format("Name")
        + "{:^40}".format("Profile Link")
        + "{:^30}".format("Date Range")
        + "{:^30}".format("Top Repo")
        + "{:^20}".format("Repo Lang")
        + "{:^30}".format("Repo Link")
        + "\n"
    ...
    for row in rows:
        ...
        repo = "{:<30}".format(row[3] if len(row[3]) < 30 else row[3][:26] + "...")
        repo_lang = "{:^20}".format(row[4] if len(row[4]) < 30 else row[4][:26] + "...")
        repo_link = "{:<30}".format(row[5] if len(row[5]) < 30 else row[5][:26] + "...")
        display_text += (
            name + profile_link + date_range + repo + repo_lang + repo_link + "\n"
        )
...
def get_URL():
    ...
    url = "https://github.com/trending/developers/{}?since={}".format(
        url_lang.lower(), url_date
    )
    return url
...
def scrape_users():
    ...
    for user in users:
        ...
        profile_link = "https://github.com{}".format(
            user.find("h1", {"class": "h3 lh-condensed"}).find("a")["href"]
        )
        repo = user.find("h1", {"class": "h4 lh-condensed"}).text.strip()
        repo_link = "https://github.com{}".format(
            user.find("h1", {"class": "h4 lh-condensed"}).find("a")["href"]
        )
...
# label for combobox
ttk.Label(
    window, text="Select Language:", background="white", font=("Helvetica", 15)
).grid(column=0, row=5, padx=10, pady=25)
ttk.Label(
    window, text="Select Date range:", background="white", font=("Helvetica", 15)
).grid(column=2, row=5, padx=10, pady=25)

# Combobox creation
language = ttk.Combobox(window, width=30, state="readonly", font="Helvetica 15")
date = ttk.Combobox(window, width=20, state="readonly", font="Helvetica 15")

# Button creation
scrape_btn = ttk.Button(
    window, text="Scrape Users!", style="my.TButton", command=scrape_users
)

display_btn = ttk.Button(
    window, text="Display from DB", style="my.TButton", command=show_results
)
