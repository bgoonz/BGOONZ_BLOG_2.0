...
def sql_insert(con, entities):
    cursorObj = con.cursor()
    cursorObj.execute(
        "INSERT INTO questions(question_text, question_summary, question_link, votes, views) VALUES(?, ?, ?, ?, ?)",
        entities,
    )
    con.commit()
...
def sql_fetch(con):
    ...
    for row in rows:
        ...
        display_text += (
            question_text + question_summary + question_link + votes + views + "\n"
        )
...
ttk.Label(
    window,
    text="Stack overflow question scraper",
    background="white",
    foreground="Orange",
...
# label texts
ttk.Label(
    window, text="Enter tag (ex - python):", background="white", font=("Helvetica", 15)
).grid(column=0, row=5, padx=10, pady=25)

ttk.Label(
    window,
    text="No of questions to scrape:",
    background="white",
    font=("Helvetica", 15),
...
scrape_btn = ttk.Button(
    window, text="Scrape questions!", style="my.TButton", command=scrape_questions
)
