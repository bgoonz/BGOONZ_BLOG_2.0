...
def sql_insert(con, entities):
    cursorObj = con.cursor()
    cursorObj.execute(
        "INSERT INTO courses(title, description, instructor, current_price, original_price, rating, hours, lectures) VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
        entities,
    )
    con.commit()
...
for course_div in course_divs:
    title = course_div.find(
        "div",
        {
            "class": "udlite-focus-visible-target udlite-heading-md course-card--course-title--2f7tE"
        },
    ).text.strip()
    ...
    original_price = course_div.find(
        "div",
        {
            "class": "price-text--price-part--Tu6MH price-text--original-price--2e-F5 course-card--list-price--2AO6G udlite-text-sm"
        },
    ).text.strip()
    original_price = original_price.replace("Original Price₹", "")

    rating = course_div.find(
        "span", {"class": "udlite-heading-sm star-rating--rating-number--3lVe8"}
    ).text.strip()

    hours = (
        course_div.find_all("span", {"class": "course-card--row--1OMjg"})[0]
        .text.strip()
        .split()[0]
    )

    lectures = (
        course_div.find_all("span", {"class": "course-card--row--1OMjg"})[1]
        .text.strip()
        .split()[0]
    )
