...
greeting = Label(
    ...
    width=40,
    height=2,
    font=("Lucida Console", 20, "italic"),
)
greeting.grid(padx=18)

# ---length of password---
lengthlabel = Label(
    Length, text="Length of Password", width=20, height=5, font=("Arial", 10, "bold")
)
lengthlabel.grid(row=5, column=1, padx=3, pady=10)

len = IntVar()
scale = Scale(
    Length, orient=HORIZONTAL, from_=6, to=24, tickinterval=1, length=500, variable=len
)
...
opt = IntVar()
c1 = Radiobutton(
    strength, text="Low", width=6, height=2, variable=opt, value=1, font=("Arial", 12)
)
...
c3 = Radiobutton(
    strength, text="High", width=6, height=2, variable=opt, value=3, font=("Arial", 12)
)
