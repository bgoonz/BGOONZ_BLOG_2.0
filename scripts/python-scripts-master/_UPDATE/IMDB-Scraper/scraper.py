...
parser.add_argument(
    "--t", action="store", type=str, required=True, help="Enter the title of the movie"
)
...
def get_info(soup):
    ...
    try:
        info["title"] = soup.find("div", attrs={"class": "title_wrapper"}).h1.get_text(
            strip=True
        )
        info["year"] = soup.find("span", attrs={"id": "titleYear"}).a.get_text(
            strip=True
        )
        info["rating"] = soup.find("span", attrs={"itemprop": "ratingValue"}).get_text(
            strip=True
        )
        ...
        info["plot"] = article.find("div", attrs={"class": "canwrap"}).p.span.get_text(
            strip=True
        )
        ...
        for block in blocks:
            ...
            if heading == "Opening Weekend USA:":
                info["opening_week_usa"] = block.get_text(strip=True).replace(
                    heading, ""
                )
    except:
...
def find_movie(query):
    ...
    if movie_list:
        for movie in movie_list:
            ...
            title_id = movie.find("td", attrs={"class": "result_text"}).a.attrs["href"][
                6:
            ]
