...
def get_snapshot_listing(options):
    ...
    if options.noop:
        ...
        if r.status_code == 200:
            ...
            return "Error in making request.  received {}: {}".format(
...
def create_snapshot(options):
    ...
    if "testing" in options and options.testing:
        indices = indexname
        snapshot_name = "{i}-{p}-{d}".format(
            i="-".join(indexname.split(",")[0].split("*")), p=period, d=timestamp
        )
    else:
        snapshot_name = "{i}-{p}-{d}".format(
            i=indexname.strip("*"), p=period, d=timestamp
        )
    ...
    payload = {
        ...
        "metadata": {
            "taken_by": getuser(),
            "taken_because": "Snapshot updated {d} for: {r}".format(
                d=timestamp, r=options.reason
            ),
        },
    }
    ...
    if options.noop:
        print(
            "Would have called url: {url}\n with payload: {pl}".format(
                url=url, pl=payload
            )
        )
        return 200
        ...
        while check_snapshot_inprogress(options):
            ...
            if options.verbose:
                print(
                    "A snapshot is running currently, script has slept {} times so far".format(
                        sleep_count
                    )
                )
            time.sleep(300)
        ...
        if snap_result.status_code == 200 and options.verbose:
            ...
            print(
                "Error encountered {code}: {response}".format(
                    code=snap_result.status_code, response=snap_result.json()
                )
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""Command line utility to take snapshots of elasticsearch indices.
    ...
    parser.add_argument(
        ...
        help="text description about why snapshot was taken",
    )
    parser.add_argument(
        "--testing",
        "-t",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Show verbose output for script",
    )
    ...
    if action == "list":
        ...
        print(
            "{action} is not a valid option.\nTry -h|--help for assistance".format(
                action=action
            )
        )
