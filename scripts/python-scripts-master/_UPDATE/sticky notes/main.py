...
class StickyNotes(Toplevel):
    def __init__(self, master, **kwargs):
        ...
        self.mainarea = tkst.ScrolledText(
            self,
            bg="#FDFDCA",
    ...
    def move_window(self, event):
        self.geometry(
            "+{0}+{1}".format(event.x_root - self.xclick, event.y_root - self.yclick)
        )
    ...
    def quit_window(self, event):
        ...
        if messagebox.askyesno(
            "Delete Note?", "Are you sure you want to delete this note?", parent=self
        ):
            global no_of_windows
            self.destroy()
            no_of_windows -= 1
