...
function for producing response of
    ...
    if (
        chat == "whodevelopedyou?"
        or chat == "whoinventedyou?"
        or chat == "developer"
        or chat == "whoisyourgod?"
    ):
        answer = "Varshith"
...
welcome = Label(
    frame_welcome, text="Welcome", font="Vardana 40 bold", bg=c1, fg="white"
)
welcome.place(x=160, y=200)

welcome_chatbot = Label(
    frame_welcome, text="I am Chatbot ! ", font="Helvetica 15 bold italic", bg=c1, fg=c6
)
welcome_chatbot.place(x=200, y=270)

welcome_chatbot = Label(
    frame_welcome,
    text="Designed by VARSHITH ",
...
label_sub = Label(
    frame_info, text="Enter Information", bg=c1, fg="white", font="Verdana 30"
)
label_sub.pack()
...
entry_user = Text(
    frame_info, bg=c6, fg="white", height="1", width="40", font="Ariel 15"
)
entry_user.focus()
...
button_1 = Button(
    frame_info, text="submit", font="Vardana 10 bold", bg=c2, fg=c1, command=info
)
button_1.place(x=470, y=330)

designer_name = Label(frame_info, text="Designed by VARSHITH", bg=c1, fg="white")
designer_name.place(x=420, y=650)

button_back = Button(
    frame_info, image=back, relief="flat", bg=c1, command=info_to_welcome
).place(x=10, y=10)
...
select_label = Label(
    frame_topic, text="Select Topic", bg=c1, fg="white", font="Ariel 30 italic"
)
select_label.pack()
...
option_1 = Label(
    frame_topic, text="1- Technology", font="Verdana 15 italic", bg=c1, fg=c2
)
option_1.place(x=30, y=120)

button_opt_1 = Button(
    frame_topic, text="Proceed", image=front, relief="flat", bg=c1, command=topic_1
)
button_opt_1.place(x=350, y=120)

option_2 = Label(
    frame_topic, text="Designed by VARSHITH", font="Verdana 8", bg=c1, fg="white"
)
option_2.place(x=410, y=650)
button_back = Button(
    frame_topic, image=back, relief="flat", bg=c1, command=topic_to_info
).place(x=10, y=10)
...
button = Button(
    bottom_frame, image=submit_img, font="Vardana 8 bold", bg=c3, command=submit
)
...
button_back = Button(
    frame_chat, image=back, relief="flat", bg=c3, command=chat_to_topic
).place(x=10, y=10)
button_front = Button(
    frame_chat, image=exitt, relief="flat", bg=c3, command=root.destroy
).place(x=440, y=10)
