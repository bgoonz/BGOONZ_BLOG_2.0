...
class WindowRecorder:
    ...
    def create_context(self):
        ...
        self.__btn_exit = Button(
            self.__app,
            text=btn_exit_txt,
            width=btn_close_width,
    ...
    def stop(self):
        ...
        file = filedialog.asksaveasfile(
            defaultextension="*.*", filetypes=[("mp4", ".mp4")]
        )
        if file:
            if file.name:
                print(file.name)
                ...
                video_writer = VideoWriter(
                    file.name, fourcc, self.__frame_per_sec, (shape[1], shape[0]), True
                )
        ...
        self.__btn_start_stop.configure(
            text=btn_start_txt, command=self.start_recording, bg="green"
        )
    ...
    def record_frames(self):
        ...
        self.min = int(dur / 60)
        self.sec = dur % 60
        self.__timer["text"] = time.strftime(
            "%H:%M:%S", time.gmtime(self.min * 60 + self.sec)
        )
