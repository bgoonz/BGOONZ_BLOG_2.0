...
for i in range(len(city_data)):
    if "deaths" in city_data[i]:
        if float(city_data[i]["deaths"]) > 50:
            folium.Marker(
                location=[city_data[i]["lat"], city_data[i]["lng"]],
                popup=city_data[i]["state"],
                icon=folium.Icon(
                    color="darkred", icon_color="white", icon="remove-sign"
                ),
                tooltip="deaths: "
                + city_data[i]["deaths"]
                + " confirmed: "
                + city_data[i]["confirmed"],
            ).add_to(mp)
            ...
            folium.Marker(
                ...
                popup=city_data[i]["state"],
                icon=folium.Icon(
                    color="orange", icon_color="white", icon="warning-sign"
                ),
                tooltip="deaths: "
                + city_data[i]["deaths"]
                + " confirmed: "
