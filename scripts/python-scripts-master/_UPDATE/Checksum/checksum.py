...
def sign(hash, message, secret):
    ...
    digest = hmac.new(
        secret.encode("utf-8"),
        msg=message.encode(),
        digestmod=hashs.get(hash, hashlib.md5),
    ).digest()
...
def main():
    ...
    parser.add_argument(
        ...
        dest="generate",
        action="store_true",
        help="Generates checksum",
    )
    parser.add_argument(
        "-s", "--sign", dest="sign", default=None, help="Signs input using HMAC"
    )
    parser.add_argument(
        "-H",
        "--hash",
    ...
    if arguments.verify is not None:
        ...
        if arguments.sign is not None:
            ...
            print(verify(arguments.hash, arguments.file.read(), arguments.verify))
            return
