...
for option in options:
    ...
    print(
        "{}. {} ({})".format(
            len(currencies), current_element["name"], current_element["short"]
        )
    )
...
amount = input(
    "\033cEnter amount of {}s (if amount isn't integer, then write it with a dot, not comma): ".format(
        currency["name"].lower()
    )
)
...
table_rows = soup.findChild("table", attrs={"class": "tablesorter"}).findChildren("tr")[
    1:
]
...
for table_row in table_rows:
    ...
    exchange_rate = {"currency": row_data[0].text, "amount": float(row_data[1].text)}
    print("{:.3f} {}s".format(exchange_rate["amount"], exchange_rate["currency"]))
