...
const executable = join(root, 'node_modules', '.bin', os.platform() !== 'win32' ? 'prettier' : 'prettier.cmd');
const ignorePath = ['--ignore-path', '.eslintignore'];
const check = process.argv.indexOf('--check') !== -1;
const mode = check ? '--list-different' : '--write';
process.chdir(root);

const { stdout, stderr, status, error } = spawnSync(executable, [...ignorePath, mode, glob]);
