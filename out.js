...
function logoForUrl(url) {
  return `<img style="margin-bottom: 0;" src="https://plus.google.com/_/favicon?domain_url=${encodeURIComponent(
    getURLOrigin(url)
  )}" alt="Logo" />`;
}
...
function generateLinksMarkdown(links) {
  ...
  const parts = links.map(
    ([name, url]) => `* [ ] [${logoForUrl(url)} ${name}](${url})`
  );
...
function generateSkillMarkdown(skill, area, collection, level) {
  ...
  let markdown = `${generateMarkdownHeading(name, level)}${PARAGRAPH_BREAK}${
    description != null
      ? `${
          description.text != null
            ? `${description.text}${PARAGRAPH_BREAK}`
            : ""
        }${generateLinksMarkdown(description.links || [])}`
      : ""
  }${LINE_BREAK}`;
  if (skills != null) {
    markdown = `${markdown}${LINE_BREAK}${generateSkillsMarkdown(
      skills,
      area,
      collection,
      level + 1
...
function generateSkillsMarkdown(skills, area, collection, level) {
  const parts = skills.map((skill) =>
    generateSkillMarkdown(skill, area, collection, level)
  );
  return parts.join(LINE_BREAK);
}
...
function generateAreaMarkdown(area, collection, level) {
  return `${
    area.name != null
      ? `${generateMarkdownHeading(area.name, level)}${PARAGRAPH_BREAK}`
      : ""
  }${generateSkillsMarkdown(area.skills, area, collection, level)}`;
}
...
function generateCollectionMarkdown(collection, level) {
  const parts = collection.areas.map((area) =>
    generateAreaMarkdown(area, collection, level + 1)
  );
  return `${generateMarkdownHeading(
    collection.name,
    level
  )}${PARAGRAPH_BREAK}${parts.join(PARAGRAPH_BREAK)}`;
}
...
function generateCollectionsMarkdown(collections) {
  const parts = collections.map((collection) =>
    generateCollectionMarkdown(collection, INITIAL_TITLE_LEVEL)
  );
...
<p align="center">
	Web Skills is a visual overview of useful skills to learn as a web developer. Go to <a href="https://bgoonz.github.io/BGOONZ_BLOG_2.0" target="_blank" aria-label="Link to Web Skills">https://bgoonz.github.io/BGOONZ_BLOG_2.0</a> to check out the visual overview or scroll through this readme to get the overview <a href="#-fundamentals" target="_blank" aria-label="Link to list of skills">as a list</a>. If you like the project you are very welcome to <a href="https://github.com/bgoonz/BGOONZ_BLOG_2.0/stargazers" aria-label="Become stargazer link">become a stargazer 🤩</a>
</p> 
<p align="center">
	<a href="http://bgoonz.github.io/BGOONZ_BLOG_2.0" target="_blank">
		<img src="https://raw.githubusercontent.com/bgoonz/BGOONZ_BLOG_2.0/master/demo.gif" alt="Web Skills Demo" width="800" />
	</a>
</p>
<br />
<details>
...
I am spending my spare time building Web Skills for free because I want to help people get into web development. My motivation comes from people finding Web Skills useful, so if you like the project feel free to support me in any way you like! For example, you are more than welcome to become a [stargazer](https://github.com/bgoonz/BGOONZ_BLOG_2.0/stargazers), share Web Skills with your friends and followers or create blog articles linking to Web Skills. If you want to, it will absolutely make my day if you [support me with a cup of coffee](https://www.buymeacoffee.com/bgoonz)! <3

### How can I get involved?

You are welcome to get involved in any way you like. If you want to, you can go to the [issues page](https://github.com/bgoonz/BGOONZ_BLOG_2.0/issues) and help me fix the spelling, fix issues or suggest some new features. Any involvement is highly appreciated!
...
The skills are based on what I personally find to be the most useful on a day-to-day basis. If you think something really important is missing, you can always suggest it on the [issues page](https://github.com/bgoonz/BGOONZ_BLOG_2.0/issues).
...
Reach out to me on Twitter at [@bgoonz](https://twitter.com/bgoonz) or take a look at [my website](https://bgoonz.github.io) if you want to learn more about what other projects I'm working on.
