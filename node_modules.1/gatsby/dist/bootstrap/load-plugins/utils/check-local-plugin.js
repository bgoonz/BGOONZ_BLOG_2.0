...
function checkLocalPlugin(plugin, rootDir) {
  const pluginName = typeof plugin === `string` ? plugin : plugin.resolve; // Make sure the plugin exists relatively
  ...
  const resolvedPackageJson = (0, _fsExistsCached.sync)(`${resolvedPath}/package.json`); // package.json is a requirement for local plugins
