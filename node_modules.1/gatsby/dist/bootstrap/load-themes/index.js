...
const resolveTheme = async (themeSpec, configFileThatDeclaredTheme, isMainConfig = false, rootDir) => {
  ...
  try {
    ...
    if (isMainConfig) {
      pathToLocalTheme = path.join(rootDir, `plugins`, themeName); // is a local plugin OR it doesn't exist
...
const processTheme = ({
  ...
  const themesList = themeConfig && themeConfig.plugins; // Gatsby themes don't have to specify a gatsby-config.js (they might only use gatsby-node, etc)
