  ...
  async _doResolvePrepareNodesQueue(type) {
    ...
    if (!_.isEmpty(actualFieldsToResolve)) {
      ...
      for (const node of (0, _datastore.getDataStore)().iterateNodesByType(typeName)) {
        ...
        const previouslyResolvedFields = (_previouslyResolvedNo = previouslyResolvedNodes === null || previouslyResolvedNodes === void 0 ? void 0 : previouslyResolvedNodes.get(node.id)) !== null && _previouslyResolvedNo !== void 0 ? _previouslyResolvedNo : {};

        const mergedResolvedFields = _.merge(previouslyResolvedFields, resolvedFields);

        resolvedNodes.set(node.id, mergedResolvedFields); // `resolvedNodesCache` redux state is always source
...
const getResolvedNodes = typeName => _redux.store.getState().resolvedNodesCache.get(typeName);
