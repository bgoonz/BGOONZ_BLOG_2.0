...
async function fetchFile({
  ...
  try {
    ...
    let filename = (0, _filenameUtils.createFilePath)(finalDirectory, name, ext); // See if there's response headers for this url
...
function getInFlightObject(key, buildId) {
  ...
  const remoteFile = (0, _getStorage.getStorage)((0, _getStorage.getDatabaseDir)()).remoteFileInfo.get(key); // if buildId match we know it's the same build and it already processed this url this build
...
async function setInFlightObject(key, buildId, value) {
  ...
  await (0, _getStorage.getStorage)((0, _getStorage.getDatabaseDir)()).remoteFileInfo.put(key, { ...value,
    buildId
  });
}