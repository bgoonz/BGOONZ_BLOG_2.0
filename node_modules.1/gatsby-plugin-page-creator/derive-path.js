...
function derivePath(path, node, reporter, slugifyOptions) {
  ...
  var errors = 0; // 1.  Incoming path can optionally contain file extension

  var modifiedPath = (0, _pathUtils.removeFileExtension)(path); // 2.  Pull out the slug parts that are within { } brackets.
  ...
  slugParts.forEach(function (slugPart) {
    ...
    var value = safeSlugify(nodeValue, slugifyOptions); // 3.e  replace the part of the slug with the actual value
